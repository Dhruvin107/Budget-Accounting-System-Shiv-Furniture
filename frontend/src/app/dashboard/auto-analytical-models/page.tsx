...
import { autoAnalyticalModelsApi, analyticalAccountsApi, aiApi } from '@/lib/api'
...
import { Plus, Search, Edit, Trash2, ToggleLeft, ToggleRight, Sparkles, Loader2 } from 'lucide-react'
...
export default function AutoAnalyticalModelsPage() {
  ...
  const [suggestingRuleValue, setSuggestingRuleValue] = useState(false)
  ...
  const handleSuggestRuleValue = async () => {
    if (!formData.name || suggestingRuleValue) return
    
    setSuggestingRuleValue(true)
    try {
      const response = await aiApi.suggestRuleValue(formData.name, formData.rule_type)
      if (response.data.suggestion) {
        setFormData({ ...formData, rule_value: response.data.suggestion })
        toast.success('AI suggestion applied!')
      } else {
        toast.error('Could not generate suggestion. AI service may not be configured.')
      }
    } catch (error) {
      toast.error('Failed to get AI suggestion')
    } finally {
      setSuggestingRuleValue(false)
    }
  }
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  onClick={handleSuggestRuleValue}
                  disabled={!formData.name || suggestingRuleValue}
              ...
              />
              <p className="text-xs text-gray-500">Enter a name above and click "AI Suggest" to auto-generate</p>
            </div>
            <div className="space-y-2">
